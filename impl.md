...
out another of its interfaces. This so called *software switch* is not
...
are so similar, we'll use the word *switch* to cover both variants
...
[Figure 1](#softswitch) shows a software switch built using a
...
in this example) using a technique called *direct memory access*
...
There are two potential bottlenecks with this approach, one or both
...
enough for a high-end router in the core of the Internet.
...
used by OSPF—are *not* directly part of the per-packet forwarding
...
The problem with ASICs is that hardware takes a long time to design
...
from the performance of ASICs and the agility of software.
...
equivalent of *open source software*. This means anyone can build a
